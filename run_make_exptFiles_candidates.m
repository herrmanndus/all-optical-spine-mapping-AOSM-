function run_make_exptFiles_targetPerm_mappedTargets(animal, session, target_groups, this_soma_location, saveDir, this_dendrite)

%% Dustin - 2021-01-22 (last update) 

saveDir = [saveDir '_SN'];

num_trials_stim = 32; %photostim trials, IMPORTANT: target_groups.num_trials has to add up to this value
num_trials_total = 64; %photostim trials plus blank trials (num_trials_total-num_trials_stim = number of blank trials)

add_blank_flag = 1;
num_blocks = 3;
stim_freq = 1/4; % 1/isi in sec 
jitter = 0;
...
%% NEEDS CHANGING DEPENDING ON EXPT DESGIN!!!
% define number of targets and trials per pattern

num_trialTypes = size(target_groups,2);
trials_perMask = (num_trials_stim*num_blocks)/num_trialTypes;

final_order = reshape(randsample(repmat(1:num_trialTypes, [1 trials_perMask]), num_trials_stim*num_blocks),[num_trials_stim num_blocks]);

% define individual pattern parameters 
num_unique_target_patterns = num_trialTypes;
cells_per_pattern = repelem(size(target_groups(1).these_coordinatesXYZ,1), size(target_groups,2));

% END: NEEDS CHANGING DEPENDING ON EXPT DESGIN!!!
...
for i = 1:num_unique_target_patterns
    ...
    SLMtargets2{1}{i} = [OffsetPoints(:,1) OffsetPoints(:,2) zumzum ones(length(zumzum),1)];
    
    SaveNames2{1}{i} = [...
        '999_'...
        num2str(i,'%04d')...
        '_' 'pattern' ...
        '_' num2str(length(zumzum)) 'Targets' ...
...
for j = 1:num_blocks
    ...
    % stimulation group
       % lookup table for small target numbers: 
    % if ntargets == 1: ask it to do 31 per cell to get 15
    % if ntargets == 2: ask if to do 20 [er cell to get 15
    % from ntargets > 2 ask it for 15 to do 15 
  
    powers_per_group_MW = 16 * cells_per_pattern;%7.5
...
%     LaserPowerPV=LaserPowerPV.*heating_dropoff_vector;
    ...
    if ~add_blank_flag
        ...
        fid = fopen([saveDir filesep 'scGroups_Triggers' num2str(j) '.dat'], 'w', 'l');
    ...
    elseif add_blank_flag
        ...
        trigger_saveDir = ['Z:\Data\M' num2str(animal) '\S' num2str(session) '\D' num2str(this_dendrite) '\sigTargets_SN'];
        ...
        % edit DH 2021-05-24: allow more than 2 consecutive blank trials
        % because reduced ITI to 4 second! (instead of 8), now less risk of
        % SLM trigger timeout: was: M = movsum(diffs,2); and any(M==2)
        success = 0;
        while ~success
...
save([saveDir filesep 'NAPARM_onlineSpineMapping_parameters.mat'],...
    ...
    'galvo_power_scale_factors','blanks_all_blocks','offset_photoToVisual','target_groups','this_dendrite',...
    'patterns_chronological_allBlocks','num_trialTypes','trials_perMask') 
